...
const isHex = (s: unknown): s is string => typeof s === 'string' && s.startsWith('0x')
const hexToBigInt = (hex: unknown): bigint => {
  if (typeof hex === 'bigint') return hex
  if (typeof hex === 'number') return BigInt(hex)
  if (isHex(hex)) return BigInt(hex)
  throw new Error(`expected hex quantity, got ${JSON.stringify(hex)}`)
}
const hexToDec = (hex: unknown): string => hexToBigInt(hex).toString(10)
const weiToXul = (wei: bigint | string | number): string => {
...
const strip0x = (hex: string): string => hex.toLowerCase().startsWith('0x') ? hex.slice(2) : hex
const pad32 = (hex: string): string => strip0x(hex).padStart(64, '0')
...
class RpcClient {
  constructor(private readonly url: string, private readonly chainId: number) {}
  ...
  async call(method: string, params: unknown[], signal: AbortSignal): Promise<unknown> {
    ...
    if (!res.ok) throw new Error(`XUL RPC HTTP ${res.status} from ${this.url}`)
  ...
  async assertChainId(signal: AbortSignal): Promise<void> {
    const id = await this.call('eth_chainId', [], signal)
    const reported = Number(hexToBigInt(id))
    if (reported !== this.chainId) {
      throw new Error(
        `XUL chain ID mismatch: configured ${this.chainId} (0x${this.chainId.toString(16)}), node reported ${reported} (0x${reported.toString(16)})`,
      )
    }
  }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const want = new Set<string>(args.fields?.length ? args.fields : ['all'])
      const needAll = want.has('all')
      const calls: Array<[string, string, string[]]> = []
      if (needAll || want.has('blockNumber')) calls.push(['blockNumber', 'eth_blockNumber', []])
      if (needAll || want.has('gasPrice')) calls.push(['gasPrice', 'eth_gasPrice', []])
      if (needAll || want.has('chainId')) calls.push(['chainId', 'eth_chainId', []])

      const raw = await Promise.all(
        calls.map(([, method, params]) => rpc.call(method, params, exec.signal)),
      )
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      return {
        ...
        balanceXul: weiToXul(balance as string),
      }
    },
    presentCall: args => ({
      card: 'generic',
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const tx: Record<string, string> = { to: args.to }
      if (args.from) tx.from = args.from
      if (args.value) tx.value = isHex(args.value) ? args.value : `0x${BigInt(args.value).toString(16)}`
      if (args.data) tx.data = args.data
      const estimate = await rpc.call('eth_estimateGas', [tx], exec.signal)
      return {
        gasEstimate: estimate as string,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const blockTag = (args.blockTag as string | undefined) ?? 'latest'
      const result = await rpc.call('eth_call', [{ to: args.to as string, data: args.data as string }, blockTag], exec.signal)
      return {
    ...
    presentCall: args => ({
      ...
      title: 'Call contract',
      kind: 'search',
      rawInput: { to: args.to, selector: (args.data as string).slice(0, 10) },
    }),
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const balanceData = `${ERC20_SELECTORS.balanceOf}${pad32(args.holderAddress as string)}`
      const [balanceHex, decimalsHex] = await Promise.all([
        rpc.call('eth_call', [{ to: args.tokenAddress as string, data: balanceData }, 'latest'], exec.signal),
        (args.withDecimals ?? true)
          ? rpc.call('eth_call', [{ to: args.tokenAddress as string, data: ERC20_SELECTORS.decimals }, 'latest'], exec.signal).catch(() => '0x12')
          : Promise.resolve('0x12'),
      ])

      const decimals = Number(hexToBigInt(decimalsHex as string))
      const raw = hexToBigInt(balanceHex as string)
      ...
      return {
        ...
        holderAddress: args.holderAddress as string,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const block = (await rpc.call('eth_getBlockByNumber', [blockTag, fullTx], exec.signal)) as {
        number?: string
        hash?: string
        timestamp?: string
        gasLimit?: string
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const stateId = (args.stateId as string | undefined) ?? 'head'
      ...
      const body = (await res.json()) as { data?: Array<{ index: string; balance: string; status: string; validator: { pubkey: string } }> }
      const all = body.data ?? []
  ...
  ctx.tools.register(defineTool({
    ...
    presentCall: args => ({
      ...
      kind: 'execute',
      rawInput: `${(args.rawTransaction as string).slice(0, 18)}… (${(args.rawTransaction as string).length - 2} bytes)`,
    }),
  }))
}
